...
# ── CLOUD CONFIG ────────────────────────────────────────────────────────────
# Set these as environment variables or create a .env file (never commit credentials).
# Example .env:
#   CLOUD_URL=https://your-app.up.railway.app
#   CLOUD_WS_URL=wss://your-app.up.railway.app
#   ADMIN_USER=your_username
#   ADMIN_PASS=your_password
CLOUD_URL    = os.environ.get("CLOUD_URL",    "https://your-cloud-app.example.com")
CLOUD_WS_URL = os.environ.get("CLOUD_WS_URL", "wss://your-cloud-app.example.com")
...
def cloud_login():
    ...
    try:
        ...
        if r.status_code == 200:
            cloud_token = r.json()["access_token"]
...
def cloud_push_detection(dt, cls_str, fname, conf):
    """Push a detection snapshot + metadata to cloud in background thread."""
    if not CLOUD_ENABLED or not cloud_token:
        return
    try:
        img_path = os.path.join(SAVE_FOLDER, fname)
        # BUG FIX: was sleeping up to 1.8 s in a hot loop — use event-based wait
        deadline = time.monotonic() + 3.0
        while not os.path.exists(img_path) and time.monotonic() < deadline:
            time.sleep(0.05)
        if not os.path.exists(img_path):
            logging.warning(f"Cloud push skipped — file never appeared: {fname}")
            return
        with open(img_path, "rb") as f:
            img_bytes = f.read()
        requests.post(
            f"{CLOUD_URL}/api/detections/push",
            headers={"Authorization": f"Bearer {cloud_token}"},
            data={
                "class_name": cls_str,
                "conf": str(conf or 0),
                "camera_idx": str(CURRENT_CAM_INDEX)
            },
            files={"image": (fname, img_bytes, "image/jpeg")},
            timeout=30
        )
        logging.info(f"Cloud push OK: {fname}")
    except Exception as e:
        logging.error(f"Cloud push error: {e}")
...
def cloud_stream_thread():
    ...
    try:
        ws_url = f"{CLOUD_WS_URL}/api/stream/pc?token={cloud_token}"
...
def capture_frames():
    ...
    while True:
        if cap is None or active_index != CURRENT_CAM_INDEX:
            ...
            print(f"Switching to camera {CURRENT_CAM_INDEX}")
            ...
            print(f"✅ Camera {CURRENT_CAM_INDEX} active")
            time.sleep(0.2)
...
def set_camera(idx):
    ...
    try:
        ...
        print("Switched to camera index", CURRENT_CAM_INDEX)
        threading.Thread(target=broadcast_settings_to_cloud, daemon=True).start()
    except Exception:
        pass
    return "OK"
...
async function nextCamera(){
  ...
  activeCamIdx=(activeCamIdx+1)%NUM_CAMERAS;
  await fetch("/set_camera/"+activeCamIdx,{method:"POST"});

  document.getElementById("feed-cam-tag").innerText="CAM "+(activeCamIdx+1);
  const cd=document.getElementById("cam-display");
  if(cd)cd.innerText="CAM "+(activeCamIdx+1);
